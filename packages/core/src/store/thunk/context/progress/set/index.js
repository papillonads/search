...
export const contextSetProgressRegularThunk = ({ message: { text, type }, isLoading = false, timeout = timeoutDefault.alert }) => async (
  dispatch,
) => {
...
export const contextSetProgressConsentThunk = ({ message: { text, type }, isLoading, consent }) => async (dispatch) => {
  const { contextSetProgressAction } = bindActionCreators(actionCreators, dispatch)
